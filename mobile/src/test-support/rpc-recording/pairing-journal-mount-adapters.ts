...
export function pairingJournalMountAdapters(
  ...
  return {
    'relay.pairing-recovery': ({ client, effect }: MountContext) => {
      const recovery = modules.load<typeof import('../../transport/mobile-relay-pairing-recovery')>(
    ...
    'pairing.pre-profile': ({ client, effect }: MountContext) => {
      const start = modules.load<typeof import('../../transport/pre-profile-pairing-coordinator')>(
