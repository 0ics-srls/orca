...
and bind the selector generation, and any failure returns C30 to migration-only. C30 itself must log
zero SQL failures. Director SQL failures are recorded as `directorSqlFailures` but do not fail the
canary, because directors log a steady baseline of `relay_cells` lock refusals unrelated to C30. Until then the
