...
export type RetainedRunReconcilerDeps = {
  ...
  canStrand?: (run: AutomationRun) => boolean
...
export class RetainedRunReconciler {
  ...
  reconcile(runs: readonly AutomationRun[], defer = false): void {
    if (this.disposed) {
      return
    }
    for (const run of runs) {
      if (defer || !this.deps.attach(run)) {
  ...
  private sweep(): void {
    ...
    for (const [runId, run] of this.pending) {
      ...
      if (strandAt !== null && Date.now() >= strandAt && this.deps.canStrand?.(run) !== false) {
        this.pending.delete(runId)
  ...
  private arm(): void {
    ...
    const settled =
      this.surfaceReadyAt !== null && Date.now() >= this.surfaceReadyAt + SURFACE_SETTLE_MS
    const timer = setTimeout(
      () => {
        this.timer = null
        this.sweep()
      },
      settled ? 30_000 : RETRY_INTERVAL_MS
    )
