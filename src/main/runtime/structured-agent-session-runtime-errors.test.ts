...
describe.each(['codex exit', 'claude exit', 'late settlement', 'journal'] as const)(
  '%s error reporting',
  (source) => {
      ...
      async (reporter) => {
        ...
        const scope = source.endsWith('exit')
          ...
          : source === 'late settlement'
            ? 'late-settlement'
            : 'journal'
        const label = source.endsWith('exit') ? 'exit recovery' : source
        ...
        for (const count of [1, 2]) {
          ...
          } else if (source === 'late settlement') {
