...
describe('buildPosixAgentHookPostCommand', () => {
  it('uses raw JSON only when the listener advertises support', () => {
    ...
    expect(command).toContain('X-Orca-Agent-Status-Run-Id: ${orca_hook_run_id}')
    expect(command).toContain('X-Orca-Agent-Status-Execution-Id: ${orca_hook_execution_id}')
    expect(command).toContain('X-Orca-Agent-Hook-Emitter-Pid: ${orca_hook_emitter_pid}')
    expect(command).toContain('--data-urlencode "emitterProcessId=$PPID"')
