...
import { callLocalRuntimeWithDeadline } from './local-runtime-call-deadline'
...
describe('local runtime call deadline', () => {
  ...
  it('declares no transport deadline for a structured-session command', async () => {
    runtimeCall.mockReturnValue(neverSettles())
    ...
    await vi.advanceTimersByTimeAsync(600_000)
    // Why: a conversation command outlives its reply. Failing the caller on elapsed transport time
    // would report an outcome nobody established; the command's terminal frame decides.
    expect(send.settled).toBe(false)
    expect(command.settled).toBe(false)
  })

  it('hands back the transport promise itself when no deadline is declared', () => {
    const transport = neverSettles()
    runtimeCall.mockReturnValue(transport)

    // Why: an extra async frame here would insert a microtask, reordering calls that callers
    // sequence against each other (terminal link activation dispatches then focuses).
    expect(callLocalRuntimeWithDeadline('terminal.focus', {}, undefined)).toBe(transport)
  })

  it('resolves an undeadlined local call within the caller microtask budget', async () => {
    vi.useRealTimers()
    ...
    void callRuntimeRpc({ kind: 'local' }, 'terminal.focus', {}).then(() => order.push('focused'))
