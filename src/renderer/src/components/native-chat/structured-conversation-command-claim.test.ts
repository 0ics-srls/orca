...
function compactItem(operationId: string, running: boolean): AgentJournalRenderItem {
  return {
    itemId: agentJournalSubmissionKey(`compact:${operationId}`),
    revision: running ? 1 : 2,
    sequence: 1,
    observedAt: 1,
    body: {
      kind: 'status',
      text: running ? 'Compacting conversation…' : 'Conversation compacted.',
      ...(running
...
describe('StructuredConversationCommandClaim', () => {
  it('settles a compaction on its terminal frame even when no reply ever arrives', async () => {
    ...
    claim.applyStreamSnapshot([compactItem(OPERATION_ID, true)])
    expect(outcome.settled).toBe(false)
    expect(claim.isRunning).toBe(true)

    claim.applyStreamSnapshot([compactItem(OPERATION_ID, false)])
