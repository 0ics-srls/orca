...
export function useNativeChatMessageRail({
  ...
  const items = useMemo(() => buildNativeChatRailItems(slots), [slots])
  ...
  useEffect(() => {
    ...
    // `items` re-runs this so a transcript that grew re-reads without waiting for
    // the next scroll; the listener itself reads everything by ref.
  }, [items, readActiveId, scrollRef])
