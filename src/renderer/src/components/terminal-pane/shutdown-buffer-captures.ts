export type ShutdownBufferCaptureOptions = {
  ...
  /** Route the captured bytes to the client-local park field instead of the shared layout.
   *  Set by the ordinary cold park, which fires on every workspace hide; the rare captures
   *  (force-park, hibernate, shutdown) stay shared so a second desktop can still cold-restore. */
  localOnly?: boolean
}
