...
/** Deliberately NOT scoped by producer: the transcript shows every agent's output.
 *  The line this module draws is that the transcript renders every item, while every
 *  "what is this agent doing right now" scan renders only the session's own agent's. */
...
/** Deliberately NOT scoped by producer: this is an existence test ("is this session
 *  listable at all"), not an attribution one. A session whose only content came from a
 *  subagent still has content, and a missing row is worse than an attributed one. */
...
/** The newest prompt the session's own user turn carries, as the sidebar quotes it.
 *  Scoped to root rows for the same reason the assistant line is: a provider that
 *  journals a subagent's own prompt would otherwise requote it as the session's. */
 ...
 *  subagent's, whose rows share this journal and are usually the newer ones while a
 *  child runs. Tool-only assistant items are skipped; the user boundary clears prose
