...
1. Default profiles remove Orca and Electron tokens from the browser engine's user agent, preserving the Chrome-shaped identity expected by imported sessions. The cleaned process identity also covers frames and workers, improving compatibility with challenge-protected sites without making the embedded browser identical to Chrome. Google sign-in hosts use a scoped Firefox identity.

If a site specifically requires Orca's native Electron identity, create a native-UA profile. Its documents and frames use the native identity, while service and shared workers keep the cleaned identity. The CLI spelling remains `orca tab profile create --no-ua-spoof` for compatibility, but the flag has this page-versus-worker limitation.
