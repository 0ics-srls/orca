...
Worker drain does not guarantee delivery of the resulting transcript after overload. The existing desktop error handler clears the active session, so later final transcripts are ignored; mobile finish rejects a session with recorded errors. Previously inserted text remains, but pending speech can be lost when this explicit error ends dictation. The proof verifies worker consumption and queue cleanup, not end-to-end transcript preservation after an error.
