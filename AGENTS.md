...
- **Linux SIGKILLs**: free memory and swap cannot tell a kernel OOM from a `systemd-oomd` kill from an outside `kill -9` — oomd kills a whole cgroup on PSI stall with gigabytes still free. Read the cgroup and PSI fields as described in [`docs/reference/linux-memory-kill-attribution.md`](./docs/reference/linux-memory-kill-attribution.md) before attributing one.
